...
phi_sample = zeros(1,2048);
[phi_T,phi_s]=BSpline(3); % Use desgined BSpline function to the scaling function and dual
phi_recon(1:length(phi_s)) = phi_s; % Get reconstruction function

phi_sample(1:length(phi_T)) = phi_T; % Get sampling function

% Calculate the inner product
len = 32;
shifted_kernels = [];
shifted_kernels_recon = [];
% Create shifted kernels
for i = 1: len
    shifted_kernels = [shifted_kernels; right_shift(phi_sample, (i - 1) * T )];
    shifted_kernels_recon = [shifted_kernels_recon; right_shift(phi_recon, (i - 1) * T )];
end
% Inner product
for i = 1: len
...
for i = 1: 4
    ...
    xlabel('t');
    ylabel('x(t)');
    title_str = ['Reproduced polynomials (Cubic B-Spline) degree ' num2str(i-1)];
    title(title_str);
end
